...
and the sync daemon itself (`src/syncd.{h,cc}`, with `src/boxsock.{h,cc}` for
the socket and `src/scanbridge.{h,cc}` for the one place another thread's work
becomes this thread's).

Still to write, roughly in order:

1. **One HCI link, shared.** The sync daemon wants to scan and to hold a
   camera, and today each half opens its own `hci::Link` over the same serial
   port -- which macOS permits, and which presents as a radio that switched
   itself off. On the Mac it is worked around by giving each radio one job
   (`--radio dongle` listens, `--radio corebluetooth` listens and leaves the
   dongle for the camera). **The box has one radio and cannot work around
   it.** What is in the way is `hcilink.h`'s one-handler-per-link model: the
   scanner and the camera both want the closed handler, and the camera also
   wants ATT and SMP.
2. **The persistence record formats** — bonds, and the roster of Tentacle
   devices seen on the network. Portable, tested, no filesystem: the box has
   NVS and nothing else, and nothing is logged to it.
3. **The boot-time delay retune.** Nothing is stored, so the Blackmagic write
...
4. **A camera backend for the Mac.** `src/camasync.h` is the interface the
...
5. **The control and state daemon.** Mac only. Aggregates any number of sync
